...
class SIMAgent:
    ...
    def judge(self,dialogue,output_format):
        ...
        if output_format is None:
            ...
            output_format = {
                ...
                "required": ["Game Summary", "Player 1 Status", "Player 2 Status", "Outcome", "Advantage"]
            }
            judge_prompt = "\n".join(dialogue) + """\n\n Evaluate the game in a very objective manner.
            Provide the following: Game Summary, Player 1 Stauts, Player 2 Status, Outcome So Far, Advantage. Nothing else.
            You are a JUDGE, you are not part of the game.
        ...
        judge_response = llmp_call(judge_prompt,
                                   judge_system_prompt,
                                   self.model,
                                   temperature=0,
                                   src='judge_call',
    ...
    def random_event(self,dialogue,output_format):
        
        if output_format:
            output_format = {
                "type": "object",
                "properties": {
                    "Random Event": {"type": "string"},
                    "Effect on Player 1": {"type": "string"},
                    "Effect on Player 2": {"type": "string"}
                },
                "required": ["Random Event", "Effect on Player 1", "Effect on Player 2"]
            }
        else:
            output_format=None
        ...
        """
        response = llmp_call(random_event_prompt,
                             random_events_system_prompt,
                             self.model,temperature=0.95, src='random_event_generator',
                             format=output_format)
        return response['message']['content']

    def sim_agent(self,user_prompt,iterations,rand_e_chance,structured=False):
        ...
        for i in range(iterations):
            ...
            if i == 0:
                ...
                # ADV response
                adv_response = self.adv_agent.adv_agent_chat(prompt,output_format)
                ...
                if np.random.random() < rand_e_chance:
                    print("Random Event")
                    if structured:
                        _random_event = self.random_event(dialogue,structured)
                    else:
                        _random_event = self.random_event(dialogue,output_format)
        ...
        print('judge')
        judge_eval = self.judge(dialogue,output_format)
        comb_dialogue.append(f"\n --- \n### Judge evaluation:\n {judge_eval}")    
        final_output = "\n".join(comb_dialogue)
        return final_output,dialogue,judge_eval,moves # for testing
        #return final_output
