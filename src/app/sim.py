...
with gr.Blocks() as demo:
        ...
        with gr.Column(scale=1):  # Buttons on the left
            with gr.Row():  # ✅ Side-by-side Trigger Button and Iterations Input
                ...
                iterations_input = gr.Textbox(
                    label="Iterations",
                    placeholder="Enter number of iterations...(default 1)",
                    lines=1
                    )
                rand_e_chance_input = gr.Textbox(
                    label="Random Events Chance",
                    placeholder="Enter the chance here...(default 100%)",
                    lines=1
                    )
    ...
    # Trigger
    #generate_btn = gr.Button("Run")
    generate_btn.click(sim_agent.sim_agent, [job_input,iterations_input,rand_e_chance_input], [response_output])
